...
extension (n: Int)
  def shown(using Show[Int]): String = render(n) // ⟹ render[Int](n)(using Show[Int])
...
val ranked = List("b" -> 2, "a" -> 1).sortBy(
  _._1
) // ⟹ type: List[Tuple2[String, Int]]; List.apply[Tuple2[String, Int]]("b" ->[Int] 2, "a" ->[Int][String] 1).sortBy(_._1)(using Ordering[String])
val labeled = nums.map(n =>
  n -> render(n)
) // ⟹ type: List[Tuple2[Int, String]]; ArrowAssoc[Int](n); render[Int](n)(using intShow)
