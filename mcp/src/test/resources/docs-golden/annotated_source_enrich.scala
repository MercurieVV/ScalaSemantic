...
extension (n: Int)
  def shown(using Show[Int]): String = render(n) // ⟹ elaborated: render[Int](n)(using Show[Int])
...
val out = render(
  nums
) // ⟹ type: String; elaborated: render[List[Int]](nums)(using listShow(using intShow))
val sorted = nums.sorted // ⟹ type: List[Int]; elaborated: nums.sorted[Int](using Ordering[Int])
val ranked = List("b" -> 2, "a" -> 1).sortBy(
  _._1
) // ⟹ type: List[Tuple2[String, Int]]; elaborated: List.apply[Tuple2[String, Int]]("b" ->[Int] 2, "a" ->[Int][String] 1).sortBy(_._1)(using Ordering[String])
val labeled = nums.map(n =>
  n -> render(n)
) // ⟹ type: List[Tuple2[Int, String]]; elaborated: ArrowAssoc[Int](n); elaborated: render[Int](n)(using intShow)
...
val firstTwo = // ⟹ type: Option[String]
  ...
  yield render(a) + render(
    b
  ) // ⟹ elaborated: render[Int](a)(using intShow); elaborated: render[Int](b)(using intShow)
