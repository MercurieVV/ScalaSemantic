...
object GenerateRegressions:
  ...
  def collectStrykerMutants(): List[String] =
    ...
    if os.exists(os.pwd / "mutation-report.json") then
      paths += os.pwd / "mutation-report.json"

    for path <- paths do
      val moduleName = if path.last == "mutation-report.json" then "analysis" else path.toIO.getParentFile.getName
      try
  ...
  def isApproved(): Boolean =
    ...
    val byCommit =
      try
        val res = os.proc("git", "log", "-1", "--pretty=%B").call()
