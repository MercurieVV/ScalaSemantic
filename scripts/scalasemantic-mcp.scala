...
object ScalaSemanticMcpScript:
  ...
  private def ensureSemanticdbConfig(project: Path, skip: Boolean): Unit =
    ...
    if sbtFiles.nonEmpty && !classpathConfigured then
      val file = project.resolve("scala-semantic.sbt")
      if !Files.exists(file) || !Files.readString(file).contains("scalaSemanticWriteClasspath") then
        Files.writeString(
          file,
        ...
        err(s"created $file")
    else if !alreadyConfigured && sbtFiles.nonEmpty then
      val file = project.resolve("scala-semantic.sbt")
