...
object SmokeTestMill {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (!Files.exists(repoRoot.resolve("build.mill"))) {
      System.err.println(
        s"Error: $repoRoot has no build.mill — run this script from the repo root."
      )
      sys.exit(1)
    }
    ...
    val semanticdbCount =
      findFiles(repoRoot.resolve("out"), _.getFileName.toString.endsWith(".semanticdb")).size
    if (semanticdbCount == 0) {
      System.err.println(
        s"Error: no *.semanticdb files under ${repoRoot.resolve("out")}. Run './mill __.compile' first."
      )
      sys.exit(1)
    }
    ...
    val buildMillSemanticdb =
      findFiles(
        repoRoot.resolve("out/mill-build"),
        _.getFileName.toString == "build.mill.semanticdb"
      ).headOption
    ...
    val cacheDir = Paths.get(
      sys.env.getOrElse("XDG_CACHE_HOME", sys.env("HOME") + "/.cache"),
      "scalasemantic-mcp"
    )
    ...
    Files.copy(
      assemblyJar,
      cacheDir.resolve("scalasemantic-mcp-local.jar"),
    ...
    println(
      "Launching server with root = repo root (real Mill project, no explicit classpath arg)..."
    )
    ...
    if (referenceCount < 1)
      fail(
        s"expected find_usages($smokeTestSymbol) to find at least 1 usage, got $referenceCount",
        stdout,
        stderr
      )
    ...
    println(
      s"find_usages OK: $referenceCount reference(s), including ${references.find(_.contains("build.mill")).get}"
    )
    ...
    if (!callPathResult("reachable").bool)
      fail(
        s"expected call_path($prePushSymbol -> $smokeTestSymbol) to be reachable, got: $callPathResult",
        stdout,
        stderr
      )
    ...
    if (path != Seq("prePush", "smokeTest"))
      fail(
        s"expected call_path to resolve exactly [prePush, smokeTest], got: $path",
        stdout,
        stderr
      )
