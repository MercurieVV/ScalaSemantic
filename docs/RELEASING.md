...
Notes are **generated, not written.** [`scripts/changelog.sh`](../scripts/changelog.sh) reads the
...
- **Docs site** ([Release notes](RELEASE_NOTES.md)) — [`scripts/gen-release-notes.sh`](../scripts/gen-release-notes.sh)
  loops every tag through `changelog.sh` to (re)build `docs/RELEASE_NOTES.md`. It is gitignored and
