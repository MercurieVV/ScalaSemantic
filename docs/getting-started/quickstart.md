...
`PreToolUse` hook, so text *search* (`Grep`, `Glob`, and shell `grep`/`rg`/`cat`/`sed`/… invoked on
a `.scala`/`.sc`/`.mill` path) is **denied** and the agent is told which MCP tool to use instead.
`Read` is left alone — `Edit`/`Write` depend on it. It fails open when
